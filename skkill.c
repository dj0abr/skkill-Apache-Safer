...
int	CHECKTIME =	10000;			// check for new log file entries every xx micro-seconds (10000 = 10ms)
...
char *skkilllog=	{"/srv/www/htdocs/skkill.log"};		// logs activities of skkill if LOG is set to 1
...
char date[100];
	...
	// ---- open all files to be scanned ----
	for(i=0; i<plog; i++) {
		fa[i]=fopen(logfiles[i],"rb");
		if(!fa[i]) 
		{
	...
	// ---- read to the end of files, ignore old contents ----
	for(i=0; i<plog; i++) 
	{
		while(fgets(line,MAXLEN,fa[i]));
	}
	...
	while(1) 
	{
		...
		{
			ignored=0;
			if(fgets(line,MAXLEN,fa[i]) != NULL) {
